 ...
 * @version 1.0
 */
public class Excel2FileDemo {
	private static Logger logger = LoggerFactory.getLogger(Excel2FileDemo.class);
	public static void main(String args[]){

		Excel2FileDemo dbdemo = new Excel2FileDemo();
		...
		excelFileConfig.setSourcePath("D:\\workspace\\bbossesdemo\\filelog-elasticsearch\\excelfiles")//指定目录
				.setFileFilter(new FileFilter() {
					...
					public boolean accept(FilterFileInfo fileInfo, FileConfig fileConfig) {
						...
						return fileInfo.getFileName().equals("works.xlsx");
					}
				});
		excelFileConfig.addCellMappingWithType(0,"works", CellMapping.CELL_NUMBER_INTEGER);//指定文件过滤器
...
//				if(v == 4 || v == 3 || v == 2){
		...
		importBuilder.setLastValueStorePath("excelworks");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
