 ...
 * @version 1.0
 */
public class Excel2FileDemo2 {
	private static Logger logger = LoggerFactory.getLogger(Excel2FileDemo2.class);
	public static void main(String args[]){

		Excel2FileDemo2 dbdemo = new Excel2FileDemo2();
		...
		excelFileConfig.setSourcePath("D:\\workspace\\bbossesdemo\\filelog-elasticsearch\\excelfiles")//指定目录
				.setFileFilter(new FileFilter() {
					...
					public boolean accept(FilterFileInfo fileInfo, FileConfig fileConfig) {
						...
						return fileInfo.getFileName().equals("auto.xlsx");
					}
				});//指定文件过滤器
//				.setSkipHeaderLines(1)//忽略第一行
		...
		importBuilder.setLastValueStorePath("excelauto");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
...
//		importBuilder.setLastValue(new Date());
		...
		importBuilder.setDataRefactor(new DataRefactor() {
			public void refactor(Context context) throws Exception  {
				Integer v = (Integer) context.getValue("works");
				if(v.intValue() == 0){
