 ...
 * @version 1.0
 */
public class ES2NewLocalFileBatchDemo {
	public static void main(String[] args){
		...
		fileFtpOupputConfig.setFilenameGenerator(new FilenameGenerator() {
			...
			public String genName( TaskContext taskContext,int fileSeq) {
				...
				return "newmetrics-report"+_fileSeq + "_"+time +"_" + _fileSeq+".txt";
			}
		});
...
//		//设置任务执行拦截器结束，可以添加多个
		//增量配置开始
		importBuilder.setLastValueColumn("newcollecttime");//手动指定日期增量查询字段变量名称
		importBuilder.setFromFirst(true);//setFromfirst(false)，如果作业停了，作业重启后从上次截止位置开始采集数据，
		//setFromfirst(true) 如果作业停了，作业重启后，重新开始采集数据
		importBuilder.setLastValueStorePath("esnew2fileftp_batchimport");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
