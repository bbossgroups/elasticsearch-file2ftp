...
public class ES2FileFtpBatchDemo {
	private static Logger logger = LoggerFactory.getLogger(ES2FileFtpBatchDemo.class);
...
//					 .setScheduleDate(date) //指定任务开始执行时间：日期
		...
		importBuilder.addCallInterceptor(new CallInterceptor() {
			...
			public void throwException(TaskContext taskContext, Throwable e) {
				logger.info("throwException 1");
				if(taskContext != null)
					logger.error(taskContext.getJobTaskMetrics().toString(),e);
			}
		});
//		//设置任务执行拦截器结束，可以添加多个
		//增量配置开始
